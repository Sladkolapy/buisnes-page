...
export default function SignInForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result?.error) {
      setError("Неверные данные. Проверьте email или телефон.");
      return;
    }
...
function GoogleButton() {
  return (
    <button
      ...
      className="flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
    >
      <GoogleIcon />
      Войти через Google
    </button>
